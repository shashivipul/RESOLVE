...
def create_signed_graph(X, Y, start, end, region=True):
    X_adjgraph=[]
    X_featgraph = []
    ...
    for i in range(len(Y)):
        ...
        cov_matrix = np.cov(bold_matrix, rowvar=False)  # Covariance matrix
        precision_matrix = np.linalg.pinv(cov_matrix)  # Inverse covariance (precision matrix)
        d = np.sqrt(np.diag(precision_matrix))  # Diagonal elements for normalization
        partial_correlation = -precision_matrix / np.outer(d, d)  # Partial correlation
        np.fill_diagonal(partial_correlation, 1.0)  # Set diagonal to 1

        if region == True:
            X_featgraph.append(partial_correlation)
        else:
            X_featgraph.append(bold_matrix)
            
        X_adjgraph.append(correlation_matrix_fisher)
