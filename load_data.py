...
def val_get_generator(sourcedata_path, dataset_name, atlas_name,training_mode):
    BOLD, labels = load_BOLD_and_labels(sourcedata_path, dataset_name, atlas_name)
    demographic_data = pd.read_csv(f'../{dataset_name}/{atlas_name}/demographics_data.csv')
...
def test_get_generator(sourcedata_path, dataset_name, atlas_name, training_mode):
    BOLD, labels = load_BOLD_and_labels(sourcedata_path, dataset_name, atlas_name)
    demographic_data = pd.read_csv(f'../{dataset_name}/{atlas_name}/demographics_data.csv')
    demographi_sex = demographics_sex= demographic_data[['Sex']].values
    _, _, test_mask = generate_masks(demographic_data, train_site, val_site, test_site) 
    filtered_sex = filter_data_by_mask(demographics_sex, test_mask)

    # Process only train data
    BOLD_filtered = filter_data_by_mask(BOLD, test_mask)
    label_filtered = filter_data_by_mask(labels, test_mask)
